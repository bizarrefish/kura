package org.eclipse.kura.core.configuration.multitons;
...
public class MultitonServiceImpl implements MultitonService, ServiceListener {
	...
	public Object[] getRegisteredMultitonInstance(String factoryPid) {
		...
		try {
			...
			s_logger.error("Could not find service for factory pid {}", factoryPid, e);
		}
	...
	public boolean removeMultitonInstance(String pid) {
		try {
			...
			s_logger.debug("Error removing instance for multiton component {}", pid);
			return false;
		}
	}
			...
			synchronized (m_callbacks) {
				for (MultitonRegistrationCallback c : m_callbacks) {
					try {
						...
						s_logger.debug("Exception while searching for instance of factory pid {}. Probably null service.pid property?", factoryPidProp);
					}
				}
			}
		}
